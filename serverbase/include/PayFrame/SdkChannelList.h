...
public:
    ...
    void getKeyData(CJsonWrapper &jr) { jr.Load(m_keyData); }
...
public:
    ...
    //          -9  超出可用的时间范围
    //          -11 调用太频繁
    bool IsValid(int32_t configId, int64_t coin, const CMyString &clientType);

    bool IsValid(int32_t configId, const CMyString &clientType);
...
public:
    ...
    int32_t GetChannel(int32_t configId, int64_t payMoney, const CMyString &clientType,
