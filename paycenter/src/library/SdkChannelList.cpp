...
bool CSdkChannel::IsValid(int32_t configId, int64_t coin, const CMyString &clientType)
{
    LogTrace("[SDK-CHANNEL]configId:%d, gameId:%d [check]configId:%d, coin:%ld, clientType:%s",
            m_configId, m_gameId, configId, coin, clientType.c_str());

    if (!this->IsValid(configId, clientType))
    {
        return false;
    }
...
bool CSdkChannel::IsValid(int32_t configId, const CMyString &clientType)
{
    ...
    case ctApple:
        ...
        {
            bValid = (m_clientFlag==clientFlag);
        }
        break;
    case ctGooglePlay:
    case ctAppStore:
    ...
    LogDebug("clientType:%s, clientFlag:%d, m_clientFlag=%d, check:%s", 
            clientType.c_str(), clientFlag, m_clientFlag, bValid ? "VALID":"INVALID");
    if (!bValid)
    {
        return false;
    }
    ...
    if (NULL == m_pSdk)
    {
        LogDebug("sdk is invalid");
        return false;
    }

    //判断sdk是否支持充值
    if (!m_pSdk->getEnableDeposit())
    {
        LogDebug("sdk: %s is Enable Deposit", m_pSdk->getCode().c_str());
        return false;
    }
...
int32_t CSdkChannelList::GetChannel(int32_t configId, int64_t payMoney, const CMyString &clientType, std::vector<CSdkChannel*> &vecChannel, int32_t &totalWeight)
{
    VEC_SDKCHANNEL::iterator it(m_vecSdkChannel.begin());
    for(; it != m_vecSdkChannel.end(); ++it)
    {
        CSdkChannel &sdkChannel = *it;
        bool val = sdkChannel.IsValid(configId, payMoney, clientType);
        if (val)
        {
            vecChannel.push_back(&sdkChannel);
...
bool CSdkChannelList::IsValid(int32_t configId, const CMyString &clientType)
{
    int32_t cnt(0);
    VEC_SDKCHANNEL::iterator it(m_vecSdkChannel.begin());
    for(; it != m_vecSdkChannel.end(); ++it)
    {
        CSdkChannel &sdkChannel = *it;
        if (sdkChannel.IsValid(configId, clientType))
        {
            ++cnt;
        }
    }
