...
CSdk::CSdk(int32_t sdkId, const CMyString& code, const CMyString& name,
        ...
        const CMyString& signCgi,const CMyString& verifyCgi)
    : m_sdkId(sdkId), m_code(code), m_name(name), m_gameCode(gameCode)
    , m_account(account), m_apiKey(apiKey), m_enableDeposit(enableDeposit), m_enableWithdraw(enableWithdraw)
{
    ...
    if (!payExtraData.empty())
    {
        ...
        CJsonWrapper jr;
        if (jr.Load(payExtraData))
        {
            m_recharge.extraData = jr.getJson();
        }
    }
    if (!payIps.empty())
    {
    ...
    m_recharge.signCgi = signCgi;
    m_recharge.verifyCgi = verifyCgi;
...
bool CSdk::IsRechargeIpValid(const CMyString &ip)
{
    return m_recharge.validIps.Find(ip);
}

bool CSdk::IsWithdrawIpValid(const CMyString &ip)
{
    return m_withdraw.validIps.Find(ip);
}
...
int32_t CSdkList::Load(ISimDB *pDB)
{
    ...
    pDB->Cmd("`withdraw_cgi`,`apiWithdrawUrl`,`apiWithdrawQueryUrl`,`apiWithdrawCallbackUrl`, ");
    pDB->Cmd("`validIps_withdraw`,`enableDeposit`,`enableWithdraw`,`sign_cgi`,`verify_cgi` ");
    ...
    while(pDB->More())
    {
        CSdk gameSdk(pDB->GetInt(0),    // id
                ...
                pDB->GetInt(17),         // enableDeposit
                pDB->GetInt(18),          // enableWithdraw
                pDB->GetString(19),         // sign_cgi
                pDB->GetString(20)          // verify_cgi
                );
