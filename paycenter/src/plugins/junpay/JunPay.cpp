...
CPluginJunPay::CPluginJunPay()
    : CPayWrapper()
{
    //mkdir("./html/jun", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
}
...
int32_t CPluginJunPay::Init(void)
{
    CMyString strSect("main/jun");
...
int32_t CPluginJunPay::Update(void)
{
    return this->Init();
}

void CPluginJunPay::Maintance()
{
...
ENTRY_FUNC_DECLARE(CPluginJunPay, doPay)
{
    int32_t nOrderId(0);
...
///////// 以下为宝丽平台专用调用
    ...
    LogDebug("[JUN]key text   [%s]", str1.c_str());
    LogDebug("[JUN]md5        [%s]", strMd5.c_str());
    LogDebug("[JUN]signed text[%s]", strSign.c_str());
    ...
    do {
        ...
        LogWarn("[JUN] header [ %s ]", strHeader.c_str());
        LogWarn("[JUN] resp   [ %s ]", strResp.c_str());
    ...
    /// 返回给调用端
    LogDebug("Jun-PayUrl:%s", strTransferUrl.c_str());
...
ENTRY_FUNC_DECLARE(CPluginJunPay, doCallback)
{
    ...
    LogInfo("JUN-pay resp, ip:[%s], data:[%s]", strIp.c_str(), strCallbackResp.c_str());
    LogImpt("JUN-pay resp, ip:[%s], data:[%s]", strIp.c_str(), strCallbackResp.c_str());
...
int32_t CPluginJunPay::dealJsonResp(CMyString &strResp, CMyString &strTransferUrl, CMyString &strError, CMyString &platformOrderNo)
{
    CJsonWrapper reader;
    ...
    {
        // 支付失败
        strError.Format("resp[%s], code:%s, error:%s", strResp.c_str(), status.c_str(), strNote.c_str());
        LogErr("[%s:%s:%d]%s", __FILE__, __func__, __LINE__, strError.c_str());
        return -1025;
    }
    CJsonWrapper data;
    ...
    {
        strError.Format("resp[%s] not has [data] field.", strResp.c_str());
        LogErr("[%s:%s:%d]%s", __FILE__, __func__, __LINE__, strError.c_str());
        return -1027;
    }
