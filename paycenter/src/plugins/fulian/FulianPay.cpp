 ...
 * =====================================================================================
 *
 *       Filename:  FulianPay.cpp
 *
 *    Description:  富连支付实现
 *
 *        Version:  1.0
 *        Created:  07/04/2020 10:12:59 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
...
CPluginFulianPay::CPluginFulianPay()
    : CPayWrapper()
{
    //mkdir("./html/fulian", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
}
...
int32_t CPluginFulianPay::Init(void)
{
    CMyString strSect("main/fulian");
...
int32_t CPluginFulianPay::Update(void)
{
    return this->Init();
}

void CPluginFulianPay::Maintance()
{
...
ENTRY_FUNC_DECLARE(CPluginFulianPay, doPay)
{
    int32_t nOrderId(0);
...
///////// 以下为宝丽平台专用调用
    ...
    req_data.add("merchant_id", pSdk->getAccount());
    req_data.add("out_trade_no", strOrderNo);
    req_data.add("pay_code", pChannel->getBankCode());
    //req_data.add("body", strUin);
    req_data.add("amount", payMoney);
    req_data.add("notify_url", strNotifyUrl);
    req_data.add("return_url", strCallbackUrl);
    //req_data.add("client_ip", strIp);

    // 生成sign
    CMyString str1 = req_data.http_str(false);
    CMyString str2("%s&key=%s", str1.c_str(), pSdk->getApiKey().c_str());
    CMyString strMd5 = CMD5Wrapper::encode(str2.c_str());
    strMd5.MakeLower();
    LogDebug("[Fulian]origin text[%s]", str1.c_str());
    LogDebug("[Fulian]with key   [%s]", str2.c_str());
    LogDebug("[Fulian]md5(key)   [%s]", strMd5.c_str());
    req_data.add("sign", strMd5);

    CMyString strSign = req_data.http_str(false);
    ...
    do {
        ...
        LogWarn("Fulian header [ %s ]", strHeader.c_str());
        LogWarn("Fulian resp   [ %s ]", strResp.c_str());
...
ENTRY_FUNC_DECLARE(CPluginFulianPay, doCallback)
{
    ...
    LogInfo("Fulian-pay resp, ip:[%s], data:[%s]", strIp.c_str(), strCallbackResp.c_str());
    LogImpt("Fulian-pay resp, ip:[%s], data:[%s]", strIp.c_str(), strCallbackResp.c_str());
    ...
    // 填充返回结果数据
    int32_t status = req.getParamInt("status");
...
int32_t CPluginFulianPay::dealJsonResp(CMyString &strResp, CMyString &strTransferUrl, CMyString &strError, CMyString &platformOrderNo, CSdkChannel *pChannel, int32_t payMoney)
{
    CGame *pGame = pChannel->getGame();
    CJsonWrapper reader;
    if (!reader.Init(strResp.c_str()))
    {
        CMyString strNote("富连支付调起结果异常, %s", strResp.c_str());
        CPayWrapper::postAlert(pGame, strNote);

        strError.Format("富连支付调用异常,请联系客服");
        // 返回串不合法
        LogErr("[%s:%s:%d]Fulian resp [%s] not valid json string", __FILE__, __func__, __LINE__, strResp.c_str());
        return -1023;
    }

    CMyString status = reader.getString("return_code");
    if (status != "SUCCESS")
    {
        CMyString strMsg = reader.getString("return_msg");
        // STEP1: 发给IM
        CMyString strNote("富连支付调起失败,错误描述:%s", strMsg.c_str());
        ...
        strError.Format("[富连支付]%s", strClientMsg.c_str());

        // STEP4: 记录日志
        LogErr("[%s:%s:%d]Fulian-Pay call failure, code:%s, error:%s", __FILE__, __func__, __LINE__, status.c_str(), strMsg.c_str());
        return -1025;
    }
    // TODO: 验签

    strTransferUrl = reader.getString("pay_url");
    CMyString strQRCode = reader.getString("qr_image_url");
