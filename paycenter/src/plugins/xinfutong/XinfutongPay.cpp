 ...
 * =====================================================================================
 *
 *       Filename:  XinfutongPay.cpp
 *
 *    Description:  信付通支付SDK
 *
 *        Version:  1.0
 *        Created:  2020年07月17日 21时47分42秒
 *       Revision:  none
 *       Compiler:  gcc
 *
...
CPluginXinfutongPay::CPluginXinfutongPay()
    : CPayWrapper()
{
    //mkdir("./html/xinfutong", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
}
...
int32_t CPluginXinfutongPay::Init(void)
{
    CMyString strSect("main/xinfutong");
...
/*
    ...
    try
    {
        m_strTransfer = xml.GetString("main/xinfutong/transfer", "url");
    }
    catch(CException &e)
    {
...
int32_t CPluginXinfutongPay::Update(void)
{
    return this->Init();
}

void CPluginXinfutongPay::Maintance()
{
...
ENTRY_FUNC_DECLARE(CPluginXinfutongPay, doPay)
{
    int32_t nOrderId(0);
...
///////// 以下为宝丽平台专用调用
    ...
    req_data.add("partner", pSdk->getAccount());
    req_data.add("service", pChannel->getBankCode());
    req_data.add("tradeNo", strOrderNo);
    req_data.add("amount", strPrice);
    req_data.add("extra", strRemark2);
    ...
    LogDebug("[XFUOO]origin text[%s]", str1.c_str());
    LogDebug("[XFUOO]key text   [%s]", str2.c_str());
    LogDebug("[XFUOO]md5        [%s]", strMd5.c_str());
    LogDebug("[XFUOO]signed text[%s]", strSign.c_str());
    ...
    //CMyString strContentType("Content-Type: multipart/form-data");
    ...
    do {
        ...
        if (ret != 0)
        {
            ...
            CMyString strNote("用户(uin=%d)调起鑫付通支付失败，错误码:%d, 错误说明:%s", strUin.c_str(), ret, strMsg.c_str() );
        ...
        LogWarn("[XFUOO] header [ %s ]", strHeader.c_str());
        LogWarn("[XFUOO] resp   [ %s ]", strResp.c_str());
...
ENTRY_FUNC_DECLARE(CPluginXinfutongPay, doCallback)
{
    ...
    LogInfo("XFuoo-pay resp, ip:[%s], data:[%s]", strIp.c_str(), strCallbackResp.c_str());
    LogImpt("XFuoo-pay resp, ip:[%s], data:[%s]", strIp.c_str(), strCallbackResp.c_str());
    
    // 校验回调IP
    CMyString strOrderNo = req.getParam("outTradeNo");
    CGame *pGame(NULL);
...
int32_t CPluginXinfutongPay::dealJsonResp(CMyString &strResp, CMyString &strTransferUrl, CMyString &strError, CMyString &platformOrderNo, CSdkChannel *pChannel, int32_t payMoney)
{
    CGame *pGame = pChannel->getGame();
    CJsonWrapper reader;
    if (!reader.Init(strResp.c_str()))
    {
        CMyString strNote("鑫付通支付调起结果异常, %s", strResp.c_str());
        CPayWrapper::postAlert(pGame, strNote);

        strError.Format("鑫付通支付调用异常,请联系客服");
        // 返回串不合法
        LogErr("[%s:%s:%d]XFUOO resp [%s] not valid json string", __FILE__, __func__, __LINE__, strResp.c_str());
        return -1023;
    }

    CMyString status = reader.getString("isSuccess");
    CMyString strCode = reader.getString("code");
    if (status != "T")
    {
        CMyString strMsg = reader.getString("msg");
        // STEP1: 发给IM
        CMyString strNote("鑫付通支付调起失败,错误描述:%s", strMsg.c_str());
        ...
        strError.Format("[鑫付通支付]%s", strClientMsg.c_str());

        // STEP4: 记录日志
        LogErr("[%s:%s:%d]XFUOO-Pay call failure, code:%s, error:%s", __FILE__, __func__, __LINE__, status.c_str(), strMsg.c_str());
        return -1025;
    }
