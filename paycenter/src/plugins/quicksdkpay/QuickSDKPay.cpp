...
CQuickSDKPay::CQuickSDKPay()
    : CPayBase("QuickSDKPay", depositRequest, httpRequestSync)
{
...
int32_t CQuickSDKPay::Init(void)
{
    int32_t ret = this->Update();
    return ret;
}

int32_t CQuickSDKPay::Update(void)
{
    CMyString strXmlSect = "main/quicksdk";
    ...
    try
    {
        pRoot = xml.GetElement("main/quicksdk/callback/key");
        while(pRoot != NULL)
        {
    ...
    {
        LogErr("get quicksdk key failure, err:%s", ex.what());
        ret = -103;
    }
    ...
    {
        LogErr("get quicksdk key failure, unknown err");
        ret = -107;
    }
...
void CQuickSDKPay::Maintance(void)
{
    CPayBase::Maintance();
}

ENTRY_FUNC_DECLARE(CQuickSDKPay, doDeposit)
{
...
ENTRY_FUNC_DECLARE(CQuickSDKPay, doDepositCB)
{
    m_strCallbackKey.clear();
    map<CMyString, CMyString>::iterator iter = m_mapCallbackKey.find("callback_tq");
...
ENTRY_FUNC_DECLARE(CQuickSDKPay, doDepositCB_hy)
{
    m_strCallbackKey.clear();
...
void CQuickSDKPay::doMakeDepositRequest(http::CHttpRequest& req, CGame* pGame, CSdk *pSdk, CSdkChannel *pChannel, const CMyString &orderNo, CMyString& url, CMyString &contentType, CMyString& httpData)
{
...
bool CQuickSDKPay::doParseDepositResp(CGame* pGame, const CMyString &respHeader, const CMyString &respData,CMyString &depositUrlData, CMyString &depositOrderNo, CMyString &errNote)
{
    return true;
}

bool CQuickSDKPay::doParseDepositCallback(http::CHttpRequest &req, CMyString &orderNo,CMyString& platformOrderNo, int64_t &fee, int64_t &realFee, int64_t &tax, bool& depositedOK)
{
    CByteStream buff;
    req.getPost(buff);
    CMyString strPost;
    ...
    CMyString strStatus;
    if (xml.getString("quicksdk_message/message/status",  strStatus) != 0)
    {
        LogDebug("[%s][%s:%d] not has quicksdk_message/message/status ", m_sdkTag.c_str(), __FILE__, __LINE__);
        return false;
    }
    ...
    CMyString strAmount;
    xml.getString("quicksdk_message/message/game_order",  orderNo);
    xml.getString("quicksdk_message/message/order_no",  platformOrderNo);
    xml.getString("quicksdk_message/message/amount",  strAmount); //用户支付金额，单位元，游戏最终发放道具金额应以此为准
...
bool CQuickSDKPay::validDepositCallbackSign(http::CHttpRequest& req, CSdk* pSdk, CSdkChannel *pChannel, CMyString& errNote)
{
...
void CQuickSDKPay::doDepositCallbackResp(bool bSucceed, int32_t errCode,std::map<CMyString, CMyString> &ext_headers, char *szResp, uint32_t& cbResp)
{
...
void CQuickSDKPay::doMakeCreditRequest(Json::Value& req, CGame* pGame, CSdk *pSdk, CMyString& url, CMyString &contentType, CMyString& httpData)
{

}

bool CQuickSDKPay::doParseCreditResp(CGame* pGame, const CMyString &respHeader, const CMyString &respData, CMyString &creditOrderNo, CMyString &errNote)
{
    return true;
}

bool CQuickSDKPay::doParseCreditCallback(http::CHttpRequest &req, CMyString &orderNo, CMyString &creditOrderNo, int64_t &fee, int64_t &realFee, int64_t &tax, bool &creditOK)
{
    return true;
}

bool CQuickSDKPay::validCreditCallbackSign(http::CHttpRequest& req, CGame *pGame, CSdk* pSdk, CMyString& errNote)
{
    return true;
}

void CQuickSDKPay::doCreditCallbackResp(bool bSucceed, int32_t errCode, std::map<CMyString, CMyString> &ext_headers, char *szResp, uint32_t& cbResp)
{
