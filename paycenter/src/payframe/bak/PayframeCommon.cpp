...
#define HTTPRESP_MIN_BUFFER     1024
...
int32_t open_mysql(const CMyString &strSect, const CMyString &strKey, ISimDB *&pDB)
{
    ...
    int32_t ret = open_mysql(strCfgName, pDB);
    if (ret < 0)
    {
        return -4+ret;
    }
...
int32_t open_redis(const CMyString &strSect, const CMyString &strKey, CMyRedis &redis)
{
    ...
    int32_t ret = open_redis(strCfgName, redis);
    if (ret < 0)
    {
        return -4+ret;
    }
...
{
    CMyString strHeader("HTTP/%s %d %s\r\n"
            "Date: %s\r\n"
            "Server: %s\r\n"
